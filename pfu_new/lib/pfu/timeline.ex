defmodule Pfu.Timeline do
  ...
  def list_deleted_posts(post) do
    Repo.all(Ecto.Query.from(p in Post, where: p.id != ^post.id, select: p))
  end

  # programado
  def inc_likes(%Post{id: id}, valor) do
    {1, [post]} =
      from(p in Post, where: p.id == ^id, select: p)
      |> Repo.update_all(inc: [likes_count: valor])
  ...
  def update_post(%Post{} = post, attrs) do
    ...
    |> Post.changeset(attrs)
    |> Repo.update()
    |> broadcast(:post_updated) #Programado
  end
